...
def emit_json(event: str, **kwargs):
...
def check_spatial_cli():
	...
	try:
		result = subprocess.run(
			['spatial', '--version'], capture_output=True, text=True
		)
...
def run_pipeline(
	...
	if not input_path.exists():
		if json_progress:
			emit_json('error', message=f'Input video not found: {input_video}')
	...
	try:
		...
		if skip_downscale:
			...
			if json_progress:
				emit_json('stage', stage='downscale', file=input_path.name)
			else:
				print('↓ Downscaling to 1080p24...')
			prepared_video = downscale_to_1080p24(
				str(input_path),
				output_path=str(work_dir / f'{input_path.stem}_1080p.mp4')
				if duration
				else None,
				duration=duration,
			)
		...
		# Stage 2: Depth + SBS
		if json_progress:
			emit_json('stage', stage='depth_stereo', file=input_path.name)
		else:
			print(
				f'◐ Generating depth & stereo (encoder={encoder[-1]}, disparity={max_disparity})...'
			)
		...
		if json_progress:
			emit_json('stage', stage='audio_mux', file=input_path.name)
		...
		if extract_audio(str(input_path), str(audio_file)):
			...
			sbs_video = mux_audio_to_video(
				str(sbs_video), str(audio_file), str(sbs_with_audio)
			)
		else:
		...
		if json_progress:
			emit_json('stage', stage='spatial_make', file=input_path.name)
		else:
			print('◉ Creating spatial video...')
		if not check_spatial_cli():
			msg = "'spatial' CLI not found. Install: brew install spatial"
			if json_progress:
		...
		cmd = [
			...
			'make',
			'-i',
			sbs_video if isinstance(sbs_video, str) else str(sbs_video),
			'-f',
			'sbs',
			'-o',
			str(output_path),
		...
		result = subprocess.run(cmd, capture_output=True, text=True)
		if result.returncode != 0:
			msg = f'spatial CLI failed: {result.stderr}'
			if json_progress:
				emit_json('error', message=msg)
			raise RuntimeError(msg)

		if json_progress:
			emit_json('done', output=str(output_path))
...
Examples:
	...
	parser.add_argument(
		'-o', '--output', help='Output path (default: output/name_spatial.mov)'
	)
	parser.add_argument(
		'--encoder',
		choices=['s', 'm', 'l'],
	...
	parser.add_argument(
		'--keep-intermediate', action='store_true', help='Keep intermediate files'
	)
	parser.add_argument(
		'--skip-downscale', action='store_true', help='Skip 1080p24 downscale'
	)
	parser.add_argument('--duration', type=float, help='Process only first N seconds')
	parser.add_argument(
		'--json-progress',
		action='store_true',
		help='Output JSON progress lines to stdout (for GUI integration)',
	)
